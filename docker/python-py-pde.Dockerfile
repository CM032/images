...
LABEL maintainer="Carlos Aznarán <caznaranl@uni.pe>" \
  name="CM032" \
  description="python-py-pde package for CM032" \
  url="https://github.com/orgs/cm032/packages/container/package/images%2Fpython-py-pde" \
...
RUN curl -s https://gitlab.com/dune-archiso/dune-archiso.gitlab.io/-/raw/main/templates/add_arch4edu.sh | bash && \
  ...
  echo "alias startJupyter=\"jupyter-lab --port=8888 --no-browser --ip=0.0.0.0 --ServerApp.allow_origin='\$(gh codespace ports --json browseUrl,sourcePort -q \'.[] | select(.sourcePort == 8888) | .browseUrl\')' --IdentityProvider.token='' --ServerApp.password=''\"" >> ~/.bashrc && \
